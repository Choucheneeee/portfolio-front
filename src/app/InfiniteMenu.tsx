...
class InfiniteGridMenu {
  ...
  private updateProjectionMatrix(): void {
    ...
    const height = this.SPHERE_RADIUS * 0.80;
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [] }) => {
  ...
  const handleButtonClick = () => {
    ...
    if (
      ...
      activeItem.link.startsWith("tel:")
    ) {
    ...
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        <>
          <div
            className={`
              absolute flex items-center gap-4 bg-[#112240]/90 border border-[#233554] rounded-xl shadow-lg px-6 py-4
              left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2
              transition-all duration-500
              ${isMoving ? "opacity-0 pointer-events-none scale-0" : "opacity-100 pointer-events-auto scale-100"}
              max-w-[90vw] w-[340px] sm:w-[400px]
            `}
          >
            <img
            ...
            <div className="flex flex-col">
              <span className="text-lg font-bold text-[#64FFDA]">{activeItem.title}</span>
              <span className="text-[#bfc9d9] text-sm">{activeItem.description}</span>
            </div>
            <button
              onClick={handleButtonClick}
              className="ml-auto flex items-center justify-center w-10 h-10 rounded-full bg-[#64FFDA] text-[#0A192F] font-bold shadow hover:bg-[#52e0c4] transition"
